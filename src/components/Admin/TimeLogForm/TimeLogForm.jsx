...
function TimeLogForm({ student, onSave, onCancel }) {
  ...
  const handleDateChange = (event, index, logType) => {
    ...
    switch (logType) {
      case "login":
        ...
        calc =
          form.values.timeStamps[index].logoutDate.getTime() - event.getTime();
        break;
      case "logout":
        form.setFieldValue(`timeStamps.${index}.logoutDate`, event);
        form.setFieldValue(`timeStamps.${index}.logoutTime`, event.getTime());
        calc =
          event.getTime() - form.values.timeStamps[index].loginDate.getTime();
        break;
      default:
        calc = 0;
        break;
  ...
  const fields = form.values.timeStamps.map((timeLog, index) => {
    ...
    if (
      Object.prototype.hasOwnProperty.call(timeLog, "_id") &&
      timeLog._id !== "" &&
      timeLog._id !== null
    ) {
      let matchingTimeStamp = student.loginTimestamps.find(
        (time) => time._id === timeLog._id
      );
        ...
        <Card shadow="sm" p="xs" mb="sm" style={{ border: border }}>
          <Group justify="space-between">
            ...
            <Button
              ...
              variant="filled"
              autoContrast
              onClick={() => form.removeListItem("timeStamps", index)}
            >
              <IconTrash />
            </Button>
          </Group>
  ...
  const handleSubmit = () => {
    ...
    onSave(updatedStudent, onCancel);
    // const updatedStudent = { ...student, loginTimestamps: updatedTimeStamps };
  };
  ...
  return (
    <Box>
      ...
      <form onSubmit={form.onSubmit(handleSubmit)} id="time-log-form">
        <Stack>
          <Box>{fields.length > 0 ? fields : <p>No time logs found.</p>}</Box>
          <Group justify="space-between">
            <Button
              type="button"
