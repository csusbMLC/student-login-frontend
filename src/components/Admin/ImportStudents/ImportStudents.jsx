...
function ImportStudents({
  onImport = () => console.log("onImport function called"),
  students = null,
  onCancel,
}) {
  ...
  onImport = () => {
        ...
        addStudent(newStudent)
          .then((response) => {
            if (response) {
              ...
              reject(
                new Error(
                  `Failed to add student: ${newStudent.studentName} with ID: ${newStudent.studentId}`
                )
              );
            }
          })
    ...
    let matchedStudentsArr = matchedStudents.map((student) => {
      const findMatchedStudent = students.find(
        (s) =>
          s.studentId === student.ID &&
          !s.classes.includes(file.name.split(".")[0])
      );
      ...
      const updatedStudent = {
        studentId: student.ID,
        studentName,
        classes: Array.from(new Set([...classes, file.name.split(".")[0]])),
        lastLogin,
        lastLogout,
        lastClass,
        loginTimestamps,
    ...
    Promise.all([...newStudentsArr, ...matchedStudentsArr])
      .then((response) => {
      ...
      .catch((error) => {
        console.error(
          "Error adding or updating students: in promise all array",
          error
        );
  ...
  const openModal = () =>
    modals.openConfirmModal({
      ...
      children: (
        <Text size="sm">
          This will import all the students listed into the database. Are you
  ...
  const parseExcel = useCallback(
    (data) => {
      ...
      if (
        sheetJSON.length === 0 ||
        sheetJSON[0].ID === undefined ||
        sheetJSON[0].Name === undefined
      ) {
        ...
        if (match) {
          ...
          if (nameMatch) {
            console.log(
              match.classes,
              file.name.split(".")[0],
              match.classes.includes(file.name.split(".")[0])
            );
          }
      ...
      {error ? (
        ...
        <>
          ...
          >
            <Table.Caption>
              {file
                ? `New students found from ${file.name}`
                : `Upload a class file to show students here`}
            </Table.Caption>
            <Table.Thead>
              <Table.Tr>
          ...
          >
            <Table.Caption>
              {file
                ? `Existing students from ${file.name}`
                : `Upload a class file to show students here`}
            </Table.Caption>
            <Table.Thead>
              <Table.Tr>
